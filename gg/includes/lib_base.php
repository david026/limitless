    ...
    else
    {
       ...
       /* $content_type = ($type == 0) ?
        ...
        else
        {
		   ...
		   ecshop 原来的邮件发送德文有乱码，david于2012/7/30 更换邮件类
            static $smtp;*/
			...
			//##########################################
			$smtpserver = $params['host']; //SMTP服务器 也可以是smtp.163.com 这个常用，除非你有企业的
			$smtpserverport = $params['port']; //SMTP服务器端口
			$smtpusermail = $params['user']; //SMTP服务器的用户邮箱
			$smtpuser = $params['user']; //SMTP服务器的用户帐号
			$smtppass = $params['pass']; //SMTP服务器的用户密码
			$smtp = new Smtp($smtpserver, $smtpserverport, true, $smtpuser, $smtppass); //这里面的一个true是表示使用身份验证,否则不使用身份验证.
			$smtp->debug = false; //是否显示发送的调试信息
##########################################
			$mailtype = "HTML"; //邮件格式（HTML/TXT）,TXT为文本邮件
			$subject = "=?UTF-8?B?".base64_encode($subject)."?=";
			$do = $smtp->sendmail($send_params['recipients'], $smtpusermail, $subject, $send_params['body'], $mailtype);
