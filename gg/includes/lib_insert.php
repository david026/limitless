...
function insert_history()
{
    ...
    if (!empty($_COOKIE['ECS']['history']))
    {
        ...
        while ($row = $GLOBALS['db']->fetch_array($query))
        {
            ...
            $str.='<li><a href="'.$goods['url'].'" ><img src="'.$goods['goods_thumb'].'" alt="'.$goods['goods_name'].'"/></a><span class="name"><a href="'.$goods['url'].'" target="_blank" title="'.$goods['goods_name'].'">'.$goods['short_name'].'</a></span><span class="price">'.$GLOBALS['_LANG']['shop_price'].'<font class="f1">'.$goods['shop_price'].'</font></span></li>';
        }
       // $str .= '<li id="clear_history" class="clear_h"><a onclick="clear_history()">' . $GLOBALS['_LANG']['clear_history'] . '</a></li>';
    }
    return $str;
}
